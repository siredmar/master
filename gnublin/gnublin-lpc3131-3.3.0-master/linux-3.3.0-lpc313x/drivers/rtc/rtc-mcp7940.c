...
static ssize_t
mcp7940_nvram_read(struct kobject *kobj, struct bin_attribute *attr,
...
static ssize_t
mcp7940_nvram_write(struct kobject *kobj, struct bin_attribute *attr,
		char *buf, loff_t off, size_t count)
{
	struct i2c_client	*client;
	struct mcp7940		*mcp7940;
...
/*
	...
	case rx_8025:
		tmp = i2c_smbus_read_i2c_block_data(mcp7940->client,
		...
		if (!(mcp7940->regs[1] & RX8025_BIT_XST)) {
			mcp7940->regs[1] |= RX8025_BIT_XST;
		...
		if (mcp7940->regs[1] & RX8025_BIT_PON) {
			mcp7940->regs[1] &= ~RX8025_BIT_PON;
			i2c_smbus_write_byte_data(client,
						  RX8025_REG_CTRL2 << 4 | 0x08,
						  mcp7940->regs[1]);
		...
		if (mcp7940->regs[1] & RX8025_BIT_VDET) {
			mcp7940->regs[1] &= ~RX8025_BIT_VDET;
			i2c_smbus_write_byte_data(client,
						  RX8025_REG_CTRL2 << 4 | 0x08,
						  mcp7940->regs[1]);
		...
		if (!(mcp7940->regs[0] & RX8025_BIT_2412)) {
			u8 hour;
			...
			// correct hour 
			hour = bcd2bin(mcp7940->regs[MCP7940_REG_HOUR]);
			if (hour == 12)
				hour = 0;
			...
			i2c_smbus_write_byte_data(client,
						  MCP7940_REG_HOUR << 4 | 0x08,
						  hour);
		}
		break;
	case ds_1388:
		mcp7940->offset = 1; // Seconds starts at 1 
		break;
	default:
		break;
	}
...
/*
	if (want_irq) {
		err = request_irq(client->irq, mcp7940_irq, IRQF_SHARED,
			  mcp7940->rtc->name, client);
		if (err) {
			dev_err(&client->dev,
		...
		set_bit(HAS_ALARM, &mcp7940->flags);
	...
	if (chip->nvram56) {
		...
		if (err == 0) {
			set_bit(HAS_NVRAM, &mcp7940->flags);
...
static int __devexit mcp7940_remove(struct i2c_client *client)
{
	...
	if (test_and_clear_bit(HAS_ALARM, &mcp7940->flags)) {
		free_irq(client->irq, client);
		cancel_work_sync(&mcp7940->work);
	}

	if (test_and_clear_bit(HAS_NVRAM, &mcp7940->flags))
