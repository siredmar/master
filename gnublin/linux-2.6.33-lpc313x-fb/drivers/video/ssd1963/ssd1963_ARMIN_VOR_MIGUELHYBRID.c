...
inline void ssd1963_send_data(struct ssd1963 *item, unsigned int data)
{
   writew(data, item->data_io);
}

static inline void ssd1963_send_cmd(struct ssd1963 *item, unsigned short reg)
{
...
//      //ssd1963_send_data(item, buffer[count]);
//   }
//
...
//   //            for (j = x; j <= xend; j++) {
//   //               ssd1963_send_data(item, buffer[j + tmpy * SCREENWIDTH]);
...
static int ssd1963_remove(struct ssd1963 *item)
{
...
static int __exit ssd1963_exit(void)
{
   pr_debug("%s\n", __func__);
