...
{
   dio_pinLevelType retval;

   retval = *(unsigned int*)(gpio_memory + FBLO_GPIO); /* Get 16 Bit Value of GPIO Inputs */
   retval = (dio_pinLevelType)((retval & (1 >> dio_pin_ui8)) & 0x01);
   return retval;
}
...
uint8 dio_readPortGroup()
{
   uint8 retval = 0;
   retval += ((dio_readChannel(DIO_CHANNEL_16) & 0x01) << 0);
   retval += ((dio_readChannel(DIO_CHANNEL_15) & 0x01) << 1);
   retval += ((dio_readChannel(DIO_CHANNEL_14) & 0x01) << 2);
   retval += ((dio_readChannel(DIO_CHANNEL_13) & 0x01) << 3);
   retval += ((dio_readChannel(DIO_CHANNEL_12) & 0x01) << 4);
   retval += ((dio_readChannel(DIO_CHANNEL_11) & 0x01) << 5);
   retval += ((dio_readChannel(DIO_CHANNEL_2)  & 0x01) << 6);
   retval += ((dio_readChannel(DIO_CHANNEL_3)  & 0x01) << 7);
   printf("dio_readPortGroup: 0x%.2X\n", retval);
   return retval;
}
