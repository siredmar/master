...
ITDB02::ITDB02(int RS, int WR,int CS, int RST, int ALE, byte model)
{ 
...
//	DDRC |= 0x03;
	...
	P_ALE	= portOutputRegister(digitalPinToPort(ALE));
	B_ALE	= digitalPinToBitMask(ALE);
	pinMode(RS,OUTPUT);
	pinMode(WR,OUTPUT);
	pinMode(CS,OUTPUT);
	pinMode(RST,OUTPUT);
