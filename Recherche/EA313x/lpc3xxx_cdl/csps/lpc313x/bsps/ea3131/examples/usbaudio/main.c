/***********************************************************************
 * $Id:: main.c 6747 2011-03-08 03:59:06Z ing02124                     $
 *
  ...
  switch (op)
  {
    case USB_SET_VOL:
      ...
      if (tmp == USB_AUDIO_VOL_SILENCE)
        tmp = UDA1380_VOLUME_SILENCE;
      else
      {
        tmp = -(int16_t)tmp / 0x40;
      }
      playback->vol = recording->vol = tmp;
      uda1380_set_volume(tmp, tmp);
...
int c_entry(void)
{
  ...
  /* Make sure USB is disabled before we start */
  USB_Connect(FALSE);
