/***********************************************************************
 * $Id:: fat.c 1757 2009-03-27 00:11:10Z pdurgesh                      $
 *
...
BOOL_32 fat_device_init(void)
{
    ...
    if (fat_data.read_func(&boot_rec[0], 0, 0) == FALSE)
    {
    ...
    if (fat_check_boot(&boot_rec[0]) == FALSE)
    {
        ...
        if (fat_check_part(&boot_rec[0]) == TRUE)
        {
            ...
            if (fat_data.read_func(&boot_rec[0],
                fat_data.fgeom.boot_part_off, 0) == FALSE)
            {
                /* Sector not readable */
                return FALSE;
            }
...
UNS_32 get_next_cluster(UNS_32 cluster)
{
    ...
    if (fat_data.clusec_cached != secoff)
    {
        /* Sector to determine the next cluster is not cached yet,
           so read it in */
        if (fat_data.read_func(&fat_data.clus_dat[0], secoff, 0) ==
            FALSE)
        {
            return FALSE;
        }
                   ...
                   void *addr)
{
    ...
    while (fat_data.filesize > 0)
    {
        ...
        if ( fat_data.filesize >=  fat_data.fgeom.cluster_size )
        {
          /* Read next sector in the cluster into memory */
          if (fat_data.read_func(saddr, sector, fat_data.fgeom.secs_cluster) == FALSE)
          {
              return FALSE;
          }
          else
          {
              ...
              saddr = saddr + (fat_data.fgeom.cluster_size / 4);
              fat_data.filesize = fat_data.filesize - fat_data.fgeom.cluster_size;
          }
        }
        else
        {
          for (idx = 0; idx < fat_data.fgeom.secs_cluster; idx++)
          {
              ...
              {
                  if (fat_data.read_func(&dump[0], sector, 0) == FALSE)
                  {
                      return FALSE;
                  }
                  else
              ...
              {
                  /* Read next sector in the cluster into memory */
                  if (fat_data.read_func(saddr, sector, 0) == FALSE)
                  {
                      return FALSE;
                  }
                  else
 ...
 *
 **********************************************************************/
BOOL_32 fat_init(bvfunc blkdev_init, bviifunc blkdev_read, 
  bvfunc blkdev_deinit)
{
...
/***********************************************************************
 *
 * Function: fat_file_read_nocache
 *
 * Purpose: Read data from an open file
 *
 * Processing:
 ...
 *
 **********************************************************************/
INT_32 fat_file_read_nocache(UNS_8 *name, UNS_8 *buff,
                     INT_32 bytes) 
{
  INT_32 file_sz = 0;
	UNS_8 fname[14];
  ...
  /* Initial cluster data is not cached */
  fat_data.clusec_cached = 0;
	if (file_track(fat_data.fbuff.next_cluster, buff) == FALSE)
    file_sz = 0;
