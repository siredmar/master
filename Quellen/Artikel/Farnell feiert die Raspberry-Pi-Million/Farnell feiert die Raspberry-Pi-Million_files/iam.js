...
var iom = iom || (function () {
  var dummySite = "dummy",
      ...
      eventList = ["","inst", "open", "clse", "play", "stop", "fowa", "bakw", "recd", "paus", "forg", "bakg", "dele", "refr", "view", "alve", "aforg", "abakg", "aclse"],
      LSOBlacklist = [],
      checkEvents = 1,
      tb = 0,
      sv = 1,
  ...
  function event(event) {
    ...
    if (inited && !isDoNotTrack() && (!checkEvents || (checkEvents && arrayContains(eventList, event)))) {
      ...
      if ( !(arrayContains(LSOBlacklist, result.st)) && ((/iPhone/.test(window.navigator.userAgent) || /iPad/.test(window.navigator.userAgent)) && /Safari/.test(window.navigator.userAgent) && !(/Chrome/.test(window.navigator.userAgent))) ) {
        baseUrl = baseUrlLSO;
        mode = 3;
