...
void ArpRequest (void)
{
	...
	// --hh (void) eth_send (NetTxPacket, (pkt - NetTxPacket) + ARP_HDR_SIZE);
}
...
NetSendUDPPacket(uchar *ether, IPaddr_t dest, int dport, int sport, int len)
{
	...
	NetSetIP (pkt, dest, dport, sport, len);
	// -hh (void) eth_send(NetTxPacket, (pkt - NetTxPacket) + IP_HDR_SIZE + len);
...
int CDPSendTrigger(void)
{
	...
	*cp = htons(chksum);
	...
	// -hh(void) eth_send(NetTxPacket, (uchar *)s - NetTxPacket);
	return 0;
}
...
NetReceive(volatile uchar * inpkt, int len)
{
	...
	case PROT_ARP:
		...
		case ARPOP_REQUEST:		/* reply with our IP address	*/
			...
			// -hh (void) eth_send((uchar *)et, (pkt - (uchar *)et) + ARP_HDR_SIZE);
			return;
		...
		case ARPOP_REPLY:		/* arp reply */
			...
			if (tmp == NetArpWaitReplyIP) {
				...
				// -hh(void) eth_send(NetArpWaitTxPacket, NetArpWaitTxPacketSize);
	...
	case PROT_IP:
		...
		if (ip->ip_p == IPPROTO_ICMP) {
			...
			case ICMP_ECHO_REQUEST:
				...
				//-hh(void) eth_send((uchar *)et, ETHER_HDR_SIZE + len);
				return;
#endif
			default:
				return;
