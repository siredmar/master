...
Die Grundidee ist, das Display über den \code{MPMC} anzuschließen, da dieser so konfiguriert werden kann, dass er sich 8080-konform verhält. Die für diese Arbeit interessanten Leitungen am Ausgang des \code{EBI} sind rot markiert. Hier wird der Datenbus selbst, sowie die oberen 13 Bit des Adressbusses gezeigt.
...
\end{lstlisting}
Der Aufruf, der die Registrierung anstößt, ist in \refl{lst:add_platform_devices} zu sehen. In der Funktion \code{ea313x\_init(void)} werden alle Hardwareeinheiten, die vom Bootloader noch nicht konfiguriert wurden, initialisiert und die verwendeten Devices im System registriert. Ohne eine vorhergehende Registrierung ist ein Verwenden eines Treibers nicht möglich.
...
In \refl{lst:setup_funktion} ist die Initialisierung des Displays zu sehen, welche mit \code{md050sd\_setup(item)} in der Probe-Funktion in \refl{lst:probe_funktion} aufgerufen wird. Prinzipiell ist der Code analog zum bereits behandelten im APEX-Bootloader in \refl{lst:apex_zweiter_teil}. Dort wird das Display in den Reset Zustand gebracht und im Anschluss mit der Farbe Schwarz beschrieben.
...
ebenfalls der Punkt \code{Console display driver support} und der Unterpunkt \code{Framebuffer Console Support} ausgewählt. Es besteht die Möglichkeit von eincompilierten Schriftarten im Linux-Kernel. Um bei der Displayauflösung von 800x480 Pixeln genügend Informationen anzeigen zu können, kann ein kleinerer Font als Standard verwendet werden z. B. \code{console 7x14 font}.
\begin{figure}[tbph]
%\begin{figure}[h!]
	\centering
