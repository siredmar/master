...
Die Grundidee ist, das Display über den \code{MPMC} anzuschließen, da dieser so konfiguriert werden kann, dass er sich 8080-konform verhält. Die für diese Arbeit interessanten Leitungen am Ausgang des \code{EBI} sind in \refa{fig:lpc_ebi} rot markiert. Hier wird der Datenbus selbst, sowie die oberen 13 Bit des Adressbusses gezeigt.
...
Der grün markierte Bereich stellt die Verbindung zum Display dar, rot die Verbindungen zum \code{Gnublin Extended} und im blauen Rechteck sind weitere kleine Bauteile untergebracht. Es sind Pullup-Widerstände mit 10~k$\Omega$ an den Leitungen STCS0, Reset und LED-A platziert, um definierte Pegel vorzugeben. Zusätzlich ist ein Blockkondensator mit 100~nF an der \code{+5\,V}-Leitung angebracht, der für eine rauscharme Spannungsversorgung des Displays sorgt.
