...
Bereits dargestellt zeigt \refa{fig:8080_pinout} auf Seite \pageref{fig:8080_pinout} das Pinout der verwendeten Displays. Der Anschluss an den Prozessor stellt sich wie in \reft{tab:display_gnublin_verbindung} dar (siehe \cite{Coldtears2014} und \cite{EmbeddedProjects2013}). Anhand der gewonnenen Erkenntnisse aus \refc{cha:teila_konzept} und \refc{cha:mpmc}, sowie des Schaltplans des verwendeten Gnublin Extended (siehe \cite{EmbeddedProjects2013}) kann eine Zuordnung getroffen werden. Nicht verbundene Pins sind mit \code{nc}\footnote{nc: not connected} vermerkt.
...
Das \code{RS} Signal am Display, welches zwischen Kommando und Daten unterscheidet, liegt auf dem Adresssignal \code{A15}. Die folgenden Angaben gehen von einer Registerkonfiguration nach \refc{cha:mpmc} aus. Werden Daten gesendet, so ist der Pin logisch 1, was einem Wert auf dem Adressbus von \code{0x1 0000}\footnote{0x1 0000 = 0b0001 0000 0000 0000 0000} entspricht. Bei Kommandos ist der Pin logisch 0 mit einem Adresswert von \code{0x00000}\footnote{0x0 0000 = 0b0000 0000 0000 0000 0000}. Die unteren 16 Bits des Adressraums lassen sich also willkürlich verändern, da nur das MSB\footnote{MSB: Most Significant Bit, das höchstwertige Bit} vom Display verwendet wird. 

Als \code{RS}-Pin ist die Adressleitung \code{A15} (logisch verschoben auf \code{A16}) gewählt, da so möglicherweise DMA-Transfers\footnote{DMA: Direct Memory Access, Speichertransfer effizient und schnell direkt in Hardware} von bis zu 65.536 Bytes\footnote{65.536 = $2^{16}$} möglich sind. Der DMA-Transfer könnte die Adressleitungen bei Daten von 0x1 0000 bis 0x1 FFFF\footnote{0x1 FFFF = 0b0001 1111 1111 1111 1111} bzw. bei Kommandos von 0x0000 bis 0x0 FFFF\footnote{0x0 FFFF = 0b0000 1111 1111 1111 1111} inkrementieren, ohne die Gültigkeit der Wahl zwischen Kommandos und Daten des Displays zu beeinträchtigen.

Das Display lässt sich zusammenfassend also über zwei Pseudoregister für Kommando und Daten auf den Adressoffsets 0x0 0000 und 0x1 0000 mit der Basisadresse 0x2000 0000 ansprechen. Dies ist in \reft{tab:sram_adressen} nochmals übersichtlich dargestellt (siehe \cite{NXP2010}).
...
Der APEX-Bootloader\footnote{\url{https://gitorious.org/apex/}} wurde ursprünglich für Prozessoren der Sharp LH Familie entwickelt, inzwischen jedoch auf eine Vielzahl von weiteren ARM basierten Prozessoren portiert - so auch für die verwendete NXP LPC313x CPU\footnote{CPU: Central Processing Unit, Prozessor}.
...
Mit der Entwicklung eines Framebuffer-Treibers, sind die Vorteile erschlossen, welche es bietet. Diese sind eine standardisierte Schnittstelle für Applikationen, sowie die Gewissheit, dass, sofern es die Rechenleistung zulässt, prinzipiell alle bereits existenten Programme und Inhalte angezeigt werden können.
